...
#define BSP_BATTERYMON_TX_ACTIVE_PIN      9
#define BSP_BATTERYMON_TX_ACTIVE_LOC      11
...
static void BatteryMonitorV2ReadADCEventHandler(uint8_t intNO);
...
void halBatteryMonitorInitialize(void)
{
  ...
  GPIOINT_CallbackRegister(BSP_BATTERYMON_TX_ACTIVE_PIN,
                           BatteryMonitorV2ReadADCEventHandler);
...
static uint32_t halBatteryMonitorReadVoltage()
{
  ...
  milliV = (uint32_t)(milliVPerBit * sample.data) * 4;
...
SL_WEAK void BatteryMonitorDataReadyCallback(uint16_t batteryVoltageMilliV)
{
}
...
// BatteryMonitorDataReadyCallback
void BatteryMonitorV2ReadADCEventHandler(uint8_t intNo)
{
  (void)intNo;
  ...
  if (timeSinceLastMeasureMS >= MS_BETWEEN_BATTERY_CHECK) {
    ...
    BatteryMonitorDataReadyCallback(voltageMilliV);
